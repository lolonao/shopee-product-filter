...
## 未解決のタスク (2025/09/10 引き継ぎ)

### ISSUE #27: UI/DB間の関心事の分離

-   [x] **UI表示項目とDBカラム名の抽象化:** Streamlitアプリ(`app.py`)の「登録済み商品検索」セクションで、UI表示する列の選択肢がDBカラム名に直接依存している。これを、内部的なカラム名とユーザーフレンドリーな表示名のマッピングを導入して抽象化する。

### ISSUE #28: 入力値の堅牢性向上

-   [ ] **計算機入力値のバリデーション:** 価格計算ツール (`calculator.py` および `app.py`) に入力値のバリデーションを追加し、範囲外の値が入力された場合にUIに警告を表示する。

---

## 機能改善 (2025/09/10 提案)

### ISSUE #29: 総合的なUI/UX改善

-   [x] **ユーザーフィードバックに基づくUI/UX改善:** 以下の4点の改善を実施。
    -   **プレビューからの価格連携:** プレビュー結果のボタンクリックで、価格と国を計算ツールに自動セット。
    -   **計算ツールの自動展開:** 上記ボタンクリック時に、計算ツールを自動で開くように変更。
    -   **CSVダウンロード機能:** 表示中の検索結果をCSVファイルでダウンロードする機能を追加。
    -   **UI微調整:** 「画像URL」をデフォルト表示列に追加し、「プレビュー」列の幅を最適化。
